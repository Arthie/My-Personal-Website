...
const NavLink: FC<NavLinkProps> = ({ href, title, active = false }) => (
  <Link href={href}>
    <a
      title={title}
      className={clsx(
        "ml-4 text-md sm:ml-6 sm:text-lg md:ml-16 md:text-xl transiton-ease font-semibold border-b-4",
        active
          ? "border-teal-100"
          : "text-teal-400 border-transparent hover:border-teal-400"
      )}
    >
      {title}
    </a>
...
const Nav: FC = () => {
  ...
  return (
    <nav
      className={clsx(
        "fixed bg-teal-900 w-full z-50 transiton-ease nav-visible",
    ...
    >
      <div className="px-4 md:px-10 m-auto my-4 flex items-center w-full max-w-xl">
        ...
        {/* <div className="w-full flex justify-end">
          <NavLink href="/" title="About" active={router.pathname === "/"} />
          <NavLink
            href="/projects"
            title="Projects"
...
const Section: FC = ({ children }) => (
  <div className="relative flex w-screen h-screen flex-col items-center max-w-lg">
    {children}
  </div>
);
